...
describe('Test the route api/student/user/log-in', () => {
    test('Should respond with a 200 status code', async () => {
        const response = await request(app).post('/api/student/user/log-in').send();
...
describe('Test the route api/student/user/sign-up', () => {
    test('Should respond with a 200 status code', async () => {
        const response = await request(app).post('/api/student/user/sign-up').send();
        expect(response.body.successful).toBe(false);
        expect(response.statusCode).toBe(200);
    });
});